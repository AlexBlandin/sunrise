...
class Sun:
  ...
  def sunrise(self, when: datetime = None):
    """
  ...
  def sunset(self, when: datetime = None):
    if when is None: when = datetime.now()
    self.__preptime(when)
    self.__calc()
    return Sun.__timefromdecimalday(self.sunset_t)
  
  def solarnoon(self, when: datetime = None):
    if when is None: when = datetime.now()
    self.__preptime(when)
    self.__calc()
  ...
  def __preptime(self, when: datetime):
    """
  ...
  def __calc(self):
    ...
    eqtime = 4 * deg(
      vary * sin(2 * rad(Mlong)) - 2 * Eccent * sin(rad(Manom)) + 4 * Eccent * vary * sin(rad(Manom)) *
      cos(2 * rad(Mlong)) - 0.5 * vary * vary * sin(4 * rad(Mlong)) - 1.25 * Eccent * Eccent * sin(2 * rad(Manom))
    )
    
    hourangle = deg(
      acos(
        cos(rad(90.833)) / (cos(rad(latitude)) * cos(rad(declination))) - tan(rad(latitude)) * tan(rad(declination))
      )
    )
